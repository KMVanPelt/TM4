...
$memberIDErr = $boardgameErr = $dateAssignedErr = $NotesErr = "";
$memberID = $boardgame = $dateAssigned = $Notes = "";
  ...
  if(!empty($_POST["boardgame"])) {
   $boardgameErr = $_POST["Boardgame selection is required"];
   } else {
   $boardgame = test_input($_POST["boardgame"]);
   }
  
  if (empty($_POST["dateAssigned"])) {
    $dateAssignedErr = "Date is required";
  } else {
    $dateAssigned = test_input($_POST["dateAssigned"]);
	// check if dateAssigned is well-formed
    if (!preg_match("/^[Date]*$/",$dateAssigned)) {
      $dateAssignedErr = "Invalid date format"; 
    }
  }
